...
Four of them take their fields from a brand board of complementary pairs. The pairs could not be used as they stand — that board sets a huge display logotype in the partner colour, where 3:1 is enough, and its red on its teal is 2.67:1: fine for a wordmark, unreadable for a line of prose. So each field is the board colour exactly, and the ink keeps the partner's *hue*, moved along lightness until the muted forms derived from it still clear 4.5:1. It still reads as teal-and-red; it is a red you can read a paragraph in. One palette, Pine, runs dark — the recessed surfaces step from the field *towards* the ink, so they lighten instead of darkening without that case being special anywhere in the CSS.
